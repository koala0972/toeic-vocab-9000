...
PROGRESS_FILE = 'tmp_gen/affiliate-one-progress.json'
...
def main():
    ...
    try:
        ...
        if e.code == 429:
            print(f'PAGE {page} rate-limited, retry next tick')
            return
        raise
    ...
    if found_count < total and not progress['done']:
        print(f'Next page: {progress["next_page"]} (next cron tick in ~11 min)')
